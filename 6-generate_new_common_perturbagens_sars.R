...
a5491_all <- generate_common_perturbagens("sars", 0.5, 0.85, "A549-10um-24h")
a5492_all <- generate_common_perturbagens("sars", 0.5, 0.85, "A549-10um-6h")
ha1e2_all <- generate_common_perturbagens("sars", 0.5, 0.85, "HA1E-10um-24h")
ht29_all <- generate_common_perturbagens("sars", 0.5, 0.85, "HT29-10um-24h")
mcf72_all <- generate_common_perturbagens("sars", 0.5, 0.85, "MCF7-10um-24h")
pc3_all <- generate_common_perturbagens("sars", 0.5, 0.85, "PC3-10um-24h")
vcap1_all <- generate_common_perturbagens("sars", 0.5, 0.85, "VCAP-10um-24h")
vcap2_all <- generate_common_perturbagens("sars", 0.5, 0.85, "VCAP-10um-6h")
...
combined <- reduce(all_data, inner_join, by = "compound") %>%
  unique() %>%
  write_csv("results/sars-new_groups-combined-drugs.csv")
