...
mcf7_all <- generate_common_perturbagens("dA549_2", 0.5, 0.85, "MCF7")
ha1e_all <- generate_common_perturbagens("dA549_2", 0.5, 0.85, "HA1E")

mcf7 <- mcf7_all %>% select(compound, meansim) %>% rename(mcf7 = meansim)
ha1e <- ha1e_all %>% select(compound, meansim) %>% rename(ha1e = meansim)

combined <- inner_join(mcf7, ha1e, by = "compound") %>% write_csv("results/dA549_2-ha1e-mcf7-combined-drugs.csv")
